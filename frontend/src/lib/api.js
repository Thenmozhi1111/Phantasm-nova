...
export async function apiFetch(path, { timeoutMs = DEFAULT_TIMEOUT_MS, ...options } = {}) {
  ...
  if (!res.ok) {
    ...
    // Attach the HTTP status so callers can branch on it (e.g. redirect to
    // login on 401) without needing the raw Response object, since apiFetch
    // already consumes and returns the parsed JSON body above.
