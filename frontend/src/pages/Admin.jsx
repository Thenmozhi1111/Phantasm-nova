...
function ForgotPasswordForm({ onBack }) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      const data = await apiFetch("/api/admin/forgot-password", {
        method: "POST",
...
function Dashboard({ onLogout }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const body = await apiFetch("/api/admin/registrations");
  ...
  const stats = data?.stats ?? {
    ...
    registrationType: { pass: 0, single: 0 },
    perEvent: [],
  };
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-10">
      ...
      {/* 1. Revenue  2. Total participants */}
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <Stat label="Total Revenue" value={inr(stats.revenue)} />
        <Stat label="Total Participants" value={String(stats.totalParticipants)} />
        <Stat label="Event Entries" value={String(stats.eventEntriesCount)} />
      </div>

      {/* 3. Veg / Non-veg  5. Single vs Nova Pass */}
      <div className="mt-4 grid gap-4 sm:grid-cols-4">
        <Stat label="Veg" value={String(stats.foodPreference.veg)} />
        <Stat label="Non-Veg" value={String(stats.foodPreference.nonVeg)} />
        <Stat label="Single Event Regs" value={String(stats.registrationType.single)} />
        <Stat label="Nova Pass Regs" value={String(stats.registrationType.pass)} />
      </div>

      {/* 4. Per-event participation */}
      <div className="mt-8">
        <h2 className="font-serif text-lg tracking-[0.2em] uppercase text-sky-100">
          Participants per Event
        </h2>
        <div className="mt-3 overflow-hidden rounded-2xl border border-sky-400/15 bg-slate-950/50">
          {stats.perEvent.length === 0 ? (
            <p className="p-5 text-center text-sm text-slate-500">No event entries yet.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-sky-400/10 text-left text-[10px] uppercase tracking-[0.2em] text-sky-400/70">
              ...
              <tbody>
                {stats.perEvent.map((e) => (
                  <tr key={e.eventName} className="border-b border-sky-400/5 last:border-0">
                    <td className="px-5 py-2.5 text-sky-100">{e.eventName}</td>
                    <td className="px-5 py-2.5 text-right font-semibold text-sky-300">
                      {e.count}
                    </td>
                  </tr>
                ))}
