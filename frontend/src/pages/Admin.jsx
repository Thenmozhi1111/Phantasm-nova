...
const onspotInputCls =
  "w-full rounded-lg border border-sky-400/20 bg-slate-950/60 px-3 py-2.5 text-sm text-sky-50 placeholder:text-slate-500 outline-none transition focus:border-sky-300 focus:ring-2 focus:ring-sky-400/30";

function OnspotRegisterForm({ onRegistered }) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      const res = await apiFetch("/api/admin/onspot-register", {
        method: "POST",
        body: JSON.stringify(form),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error ?? "Could not register participant.");
        return;
      }
      setResult(data);
      setForm({ name: "", email: "", phone: "", collegeName: "", eventId: "" });
      onRegistered?.();
    } catch {
      setError("Network error. Please try again.");
    } finally {
  ...
  return (
    <div className="rounded-2xl border border-sky-400/15 bg-slate-950/50 p-5 shadow-[0_0_30px_rgba(56,189,248,0.08)]">
      <h2 className="font-serif text-lg tracking-wider text-sky-100">On-Spot Registration</h2>
      <p className="mt-1 text-xs text-slate-400">
        Register a walk-in participant. Payment is collected separately at the desk — this entry
      ...
      {result && (
        <div className="mt-4 rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-4 text-center">
          ...
          <p className="mt-1 font-mono text-2xl font-semibold tracking-wider text-emerald-200">
            {result.phantasmId}
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <select
          ...
          required
          className={`${onspotInputCls} sm:col-span-2`}
        >
        ...
        {error && (
          <p className="rounded-lg border border-red-400/30 bg-red-500/10 px-3 py-2 text-xs text-red-300 sm:col-span-2">
            {error}
          </p>
        )}
        ...
        <button
          type="submit"
          disabled={submitting}
          className="rounded-full border border-sky-400/50 bg-gradient-to-b from-sky-500/30 to-sky-950/60 px-6 py-3 font-semibold uppercase tracking-[0.2em] text-sky-100 shadow-[0_0_30px_rgba(56,189,248,0.35)] transition hover:from-sky-400/40 disabled:opacity-40 sm:col-span-2"
        >
    ...
    <div className="mx-auto max-w-5xl px-5 py-10">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => setShowOnspotForm((v) => !v)}
            className="rounded-full border border-emerald-400/40 px-4 py-1.5 text-xs uppercase tracking-widest text-emerald-300 hover:border-emerald-400/70 hover:text-emerald-100"
          >
            {showOnspotForm ? "Close" : "+ On-Spot Registration"}
          </button>
          <Link
            to="/"
      ...
      {showOnspotForm && (
        <div className="mt-6">
          <OnspotRegisterForm onRegistered={loadRegistrations} />
        </div>
      )}
