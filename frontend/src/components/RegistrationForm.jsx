...
export default function RegistrationForm() {
  ...
  /*
   * Team rosters for SINGLE-EVENT mode only.
   *
   * Shape: { [eventId]: { teamName, members: [{ name, email, phone }] } }
   *
   * `members` holds the OTHER members — the registrant filling this form
   * is always the team leader and is counted automatically, so the real
   * team size is members.length + 1.
   *
   * Nova Pass never touches this state.
   */
  ...
  /*
   * Team events that need a roster — single-event mode only.
   */
  ...
  function blankTeam(ev) {
    /*
     * Pre-fill the minimum number of extra member rows. The leader
     * counts as 1, so minSize 3 means 2 rows to fill in.
     */
  ...
  const setMemberField = useCallback(
    (eventId, index, field, value) => {
      setTeams((p) => {
        const team = p[eventId];
        if (!team) return p;
  ...
  const addMember = useCallback(
    (ev) => {
      setTeams((p) => {
        ...
        // leader + existing members must stay under maxSize
        if (
  ...
  function handlePay() {
    ...
    if (
      ...
      setError(
        "Please choose the food preference.",
      );
      return;
    }

    /*
     * Team rosters — single-event mode only.
     */
    for (const ev of teamEvents) {
      const team =
        teams[ev.id] ?? blankTeam(ev);

      if (!team.teamName.trim()) {
        setError(
          `Enter a team name for ${ev.name}.`,
        );
        return;
      }
      ...
      if (
        ...
        setError(
          ev.minSize === ev.maxSize
            ? `${ev.name} needs exactly ${ev.maxSize} members (including you).`
            : `${ev.name} needs ${ev.minSize}–${ev.maxSize} members (including you).`,
        );
        return;
      }
      ...
      for (
        ...
        if (
          ...
          setError(
            `Fill in name, email and phone for member ${i + 2} of ${ev.name}.`,
          );
          return;
        }
      }
  ...
  async function doSubmit() {
    ...
    const payload = {
      ...
      events: selectedEvents.map((ev) => {
        /*
         * Nova Pass sends nothing extra — unchanged behaviour.
         */
        if (isPass || ev.type !== "team") {
        ...
        return {
          ...
          // The leader (you) is added on the server from the
          // contact details, so only the others go here.
          teamMembers: team.members.map(
            (m) => ({
              name: m.name.trim(),
              email: m.email.trim(),
              phone: m.phone.trim(),
    ...
    try {
       ...
       * STEP 1
       *
       * Create the registration in your backend.
       */
       ...
       *
       * The Razorpay secret is NEVER sent to the frontend.
       */
  ...
  async function triggerRazorpayCheckout(
    ...
    try {
      ...
      const options = {
        ...
        description: isPass
          ? "Phantasm 2026 Fest Pass"
          : "Phantasm 2026 Event Registration",
        ...
        modal: {
          ondismiss: () => {
            ...
            setError(
              "Payment window was closed. You can reopen Razorpay Checkout and try again.",
            );
          },
        },

        /*
         * Razorpay calls this only after successful Checkout.
         */
        handler: async (
          response,
        ) => {
  ...
  /*
   * Send Razorpay's successful response
   * to our backend.
   */
  async function confirmPayment(
    ...
    if (
      ...
      setError(
        "Razorpay did not return the required payment details. Please try again.",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        setError(
          "Payment is still being processed. Please check the status again in a moment.",
        );
      }
    } catch (err) {
      setError(
    ...
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-sky-400/30 bg-slate-950/70 p-10 text-center shadow-[0_0_60px_rgba(56,189,248,0.25)] backdrop-blur">
        ...
        <p className="mt-4 text-xs text-slate-500">
          ...
          your event, team and payment
          details has been sent to your
          registered email address. Also check your spam folder if you don't see it in your inbox.
        </p>
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
      <div className="space-y-6">
        ...
        <Panel
          title="Your Details"
          subtitle="These details are used for your registration and payment confirmation."
        >
        ...
        <Panel
          ...
          subtitle={
            isPass
              ? "The pass remains ₹300 and unlocks up to 2 tech + 2 non-tech events. Events in the same row clash."
              : "Select any number of events within the parallel constraints. Solo events are ₹150 each. Team events are a single ₹200 payment for the whole team (DataLens stays at ₹150)."
          }
        >
          {isPass && (
        ...
        {/*
          * Team details — SINGLE EVENTS ONLY.
          * Nova Pass never renders this block.
          */}
        {!isPass &&
          teamEvents.length > 0 && (
            <Panel
              title="Team Details"
              subtitle="One payment covers your whole team. You are counted as the team leader — add the rest of your members below."
            >
              <div className="space-y-6">
                {teamEvents.map((ev) => {
                  const team =
                  ...
                  return (
                    <div
                      key={ev.id}
                      className="rounded-xl border border-amber-400/20 bg-slate-950/50 p-5"
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <h3 className="font-serif text-base uppercase tracking-[0.2em] text-amber-100">
                          <span className="mr-2">
                            {ev.emoji}
                          </span>
                          {ev.name}
                        ...
                        <span className="rounded-full border border-amber-400/30 bg-amber-500/10 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-widest text-amber-300">
                          {ev.minSize ===
                          ev.maxSize
                            ? `Team of ${ev.maxSize}`
                      ...
                      <div className="mt-4 rounded-lg border border-sky-400/15 bg-black/20 px-3 py-2.5">
                      ...
                      {team.members.map(
                        (m, i) => (
                          <div
                            key={i}
                            className="mt-4 rounded-lg border border-sky-400/10 bg-black/20 p-3"
                          >
                            <div className="flex items-center justify-between">
                              ...
                              {canRemove && (
                                <button
                                  ...
                                  className="text-[10px] font-semibold uppercase tracking-widest text-red-300/70 transition hover:text-red-300"
                                >
                                  Remove
                                </button>
                              )}
                      ...
                      {canAdd && (
                        <button
                          ...
                          className="mt-4 w-full rounded-lg border border-dashed border-amber-400/30 px-4 py-2.5 text-[11px] font-semibold uppercase tracking-widest text-amber-300/80 transition hover:border-amber-400/60 hover:text-amber-200"
                        >
                          + Add member (up
                          to {ev.maxSize})
                        </button>
        ...
        <div className="relative rounded-2xl border border-sky-400/20 bg-slate-950/70 p-6 shadow-[0_0_40px_rgba(56,189,248,0.15)] backdrop-blur">
          ...
          <div className="relative mt-4 space-y-2 text-sm">
            ...
            {selectedEvents.map(
              (e) => (
                <div
                  key={e.id}
                  className="flex justify-between gap-3 text-sky-100/80"
                >
                  <span>
                    <span className="mr-1">
                      {e.emoji}
          ...
          <div className="relative mt-5 rounded-xl border border-sky-400/15 bg-slate-950/50 p-4">
            ...
            <p className="mt-1 text-[11px] leading-relaxed text-slate-500">
              {isPass
                ? "Claim a food pass based on your choice"
            ...
            <div className="mx-4 w-full max-w-md rounded-2xl border border-sky-400/40 bg-slate-950/95 p-8 shadow-[0_0_60px_rgba(56,189,248,0.35)]">
              ...
              <p className="mt-4 rounded-lg border border-sky-400/20 bg-sky-500/5 p-3 text-xs text-sky-200">
                {submitting
                  ...
                  : "Complete your payment in the Razorpay window. If it didn't open, tap Reopen Checkout below."}
              </p>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 backdrop-blur-sm">
          <div className="mx-4 max-w-md rounded-2xl border border-amber-400/40 bg-slate-950/95 p-8 shadow-[0_0_60px_rgba(251,191,36,0.25)]">
            ...
            <p className="mt-2 text-center text-xs text-slate-500">
              Return to the gate and
              claim your full share, or
              press on.
            </p>
