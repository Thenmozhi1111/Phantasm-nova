...
/* ---------------------------------------------------------
   EVENT TIMINGS
--------------------------------------------------------- */
function getEventTiming(eventName) {
  ...
  if (name === "techtrinity" || name === "innosphere") {
    return "10:30 AM – 11:30 AM";
  }

  if (name === "visionforge" || name === "datalens") {
    return "11:30 AM – 12:30 PM";
  }

  if (name === "zonein" || name === "mindwar") {
    return "1:30 PM – 2:30 PM";
  }

  if (name === "questexe" || name === "bidpro") {
    return "2:30 PM – 3:30 PM";
  }

  return "Timing will be announced";
}
...
export async function sendRegistrationConfirmationEmail({
  ...
  const eventRows = events
    .map((e) => {
      const timing = getEventTiming(e.eventName);
      ...
      /*
       * Do not show ₹0 for events when the participant
       * registered using the Full Fest Pass.
       *
       * For individual event registrations, the amount
       * will continue to be displayed.
       */
      const amountDetail = isPass
        ? ""
        : ` (₹${e.amount})`;

      return `<li style="margin-bottom:16px;line-height:1.5;">
        <strong>${e.eventName}</strong>${amountDetail}

        <br>
        <span style="font-size:12px;color:#0284c7;margin-left:12px;display:inline-block;font-weight:bold;">
          🕒 Time: ${timing}
        </span>

        ${teamDetail}${laptopDetail}
      </li>`;
    })
    .join("");
  ...
  const html = layout(
    ...
    <p style="margin-top:20px;">
      Keep your Phantasm ID handy — you'll need it at check-in and if any teammate wants to look up your team when registering.
    </p>

    <!-- IMPORTANT POINTS -->

    <div style="margin-top:28px;padding:18px;background-color:#f0f9ff;border:1px solid rgba(2,132,199,0.25);border-radius:12px;">

      <p style="margin:0 0 12px;font-size:14px;font-weight:bold;color:#0284c7;letter-spacing:0.5px;">
        IMPORTANT POINTS
      </p>

      <ol style="margin:0;padding-left:20px;color:#475569;font-size:13px;line-height:1.8;">
        <li>
          Please be present by <strong>9:00 AM</strong> for the events.
        </li>

        <li>
          <strong>Laptop is optional.</strong>
        </li>

        <li>
          <strong>Stable internet will be provided.</strong>
        </li>
      </ol>
...
Registration: ${isPass ? "Full Fest Pass" : "Single Event(s)"}
...
${events
  .map((e) => {
    ...
    /*
     * Don't show the event amount when it is
     * included in the Full Fest Pass.
     */
    const amountDetail = isPass
      ? ""
      : ` - Rs. ${e.amount}`;
    ...
    const teamDetail = e.teamName
      ? ` - Team: ${e.teamName}${
          e.teamMembers && e.teamMembers.length
            ? ` - Teammates: ${e.teamMembers.join(", ")}`
            : ""
        }`
      : "";

    const laptopDetail = ` - Laptop: ${
      e.requiresLaptop ? "Required" : "Optional"
    }`;

    return `- ${e.eventName}${amountDetail}
  Timing: ${timing}${teamDetail}${laptopDetail}`;
  })
  .join("\n")}

Please keep your Phantasm ID handy for check-in.

IMPORTANT POINTS:
1. Please be present by 9:00 AM for the events.
2. Laptop is optional.
3. Stable internet will be provided.
`;

  return sendMail({
    to,
    subject: "Phantasm 2026 — Registration Confirmed",
    html,
    text,
  });
}
...
export async function sendAdminPasswordResetEmail({
  ...
  const html = layout(
    ...
    <p style="word-break:break-all;font-size:12px;color:#0284c7;">
      ${resetUrl}
    </p>
...
If you didn't request this, you can safely ignore this email.`;
  ...
  return sendMail({
    to,
    subject: "Phantasm 2026 — Reset your admin password",
    html,
    text,
  });
}
...
export async function sendAdminPasswordChangedEmail({ to }) {
  ...
  return sendMail({
    to,
    subject: "Phantasm 2026 — Admin password changed",
    html,
    text,
  });
}
