...
export async function listRegistrations(req, res, next) {
  try {
    ...
    // needs_accommodation is repurposed as the food preference at
    // registration time: 'yes' = veg, 'no' = non-veg (see registrationController).
    let vegCount = 0;
    let nonVegCount = 0;
    let passCount = 0;
    let singleCount = 0;
    for (const r of paidRegs) {
      if (r.needs_accommodation === "yes") vegCount += 1;
      else if (r.needs_accommodation === "no") nonVegCount += 1;

      if (r.pass_type === "pass") passCount += 1;
      else singleCount += 1;
    }

    // Per-event participation count: one row per student per event entry.
    const perEventMap = new Map();
    for (const r of paidRegs) {
      for (const e of r.events) {
        perEventMap.set(e.event_name, (perEventMap.get(e.event_name) ?? 0) + 1);
      }
    }
    ...
    const stats = {
      ...
      totalParticipants: paidRegs.length,
      eventEntriesCount: paidRegs.reduce((sum, r) => sum + r.events.length, 0),
      revenue: paidRegs.reduce((sum, r) => sum + r.total_amount, 0),
      foodPreference: { veg: vegCount, nonVeg: nonVegCount },
      registrationType: { pass: passCount, single: singleCount },
      perEvent,
    };
