...
const eventSelectionSchema = z.object({
  ...
  // Additional team members ONLY (the registrant/leader is taken from the
  // contact fields on the server, never from the client payload).
  teamMembers: z.array(participantSchema).optional(),
  // Legacy field name, still accepted so older clients keep working.
...
export const registrationSchema = z.object({
  ...
  needsAccommodation: z.enum(["yes", "no"], {
    errorMap: () => ({ message: "Please choose the food preference." }),
  }),
...
function resolveEvents(payload) {
  ...
  for (const sel of payload.events) {
    ...
    // Nova Pass keeps its original behaviour: no team name, no roster —
    // the pass holder simply registers themselves for each event.
    if (payload.pass || ev.type !== "team") {
      resolved.push({ ev, teamName: null, participants: [leader] });
      continue;
    }

    // Single-event mode, team event: one payment covers the whole team,
    // so the full roster is collected and validated here.
    const teamName = (sel.teamName ?? "").trim();
    if (!teamName) {
      throw badRequest(`Team name is required for ${ev.name}.`);
    }

    const extras = sel.teamMembers ?? sel.participants ?? [];
    const roster = [leader, ...extras];

    if (roster.length < ev.minSize || roster.length > ev.maxSize) {
      const range =
        ev.minSize === ev.maxSize
          ? `exactly ${ev.maxSize}`
          : `between ${ev.minSize} and ${ev.maxSize}`;
      throw badRequest(
        `${ev.name} needs ${range} team members (including you). You sent ${roster.length}.`,
      );
    }
    ...
    for (const p of roster) {
      ...
      if (emails.has(key)) {
        throw badRequest(`Duplicate team member email in ${ev.name}: ${p.email}`);
      }
      emails.add(key);
    }

    resolved.push({ ev, teamName, participants: roster });
  }
